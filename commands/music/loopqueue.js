...
module.exports = {
    name: 'loopqueue',
    description: 'Loop cả toàn bộ danh sách chờ',
    aliases: ['lq'],
    //cooldown: '',
    /** 
     * @param {Client} client 
    ...
    run: async(client, message, args) => {
        ...
        try {
            ...
            if (player.queueRepeat) {
                ...
                message.channel.send(new MessageEmbed().setDescription(`:repeat: Lặp lại toàn bộ playlist \`tắt\``).setColor('RANDOM'));
            } else {
                player.setQueueRepeat(true)
                message.channel.send(new MessageEmbed().setDescription(`:repeat: Lặp lại toàn bộ playlist \`bật\``).setColor('RANDOM'));
            }
        } catch (error) {
            console.log(error)
